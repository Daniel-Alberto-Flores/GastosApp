...
{		
	public partial class MainForm : Form
	{
		...
		{
			if (tb_egre_new_tipo.Text.Trim().Length <= 14)
            {
            ...
            {
				MessageBox.Show("Solo se permiten ingresar hasta 14 carácteres para el campo nombre.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
			}							
		}
		
		void Dgv_list_egre_dia_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			if ((e.RowIndex > -1) && (egreso != ""))
			{
		...
		{
			int dotCount = tb_egre_monto.Text.Where(x => (x == '.')).Count();//Obtenemos la cantidad de "." cargados
			bool verifyStr = Double.TryParse(tb_egre_monto.Text, out _);//Devolvemos true si es un valor numérico
            ...
            {
				MessageBox.Show("Valor ingresado incorrecto.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
			}						
		}
		...
		{
			if (tb_ingre_new_tipo.Text.Trim().Length <= 14)
            {
		...
		{
			int dotCount = tb_ingre_monto.Text.Where(x => (x == '.')).Count();//Obtenemos la cantidad de "." cargados
			bool verifyStr = Double.TryParse(tb_ingre_monto.Text, out _);//Devolvemos true si es un valor numérico
			if (verifyStr && (dotCount < 2))
            {
		...
		{
			string notasPath = System.Environment.CurrentDirectory + "//notas.txt";
			if (File.Exists(notasPath))
            {		
            	string[] lines = File.ReadAllLines(notasPath);
            	int conta =1;
		...
		private void met_update_notas()
		{
			...
			if (File.Exists(notasPath))
            {
            	...
            	System.IO.StreamWriter file = new System.IO.StreamWriter(notasPath);
		        foreach (string line in lines)
		        {
		        	file.WriteLine(line);
		        }
